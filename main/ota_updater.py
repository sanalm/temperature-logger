...
class OTAUpdater:
    ...
    def _download_and_install_update(self, latest_version, ssid, password):
        ...
        if 'backup' in os.listdir(self.module):
            self.rmtree(self.modulepath('backup'))    
        os.mkdir(self.modulepath('backup'))
        os.rename(self.modulepath(self.main_dir), self.modulepath('backup'))
